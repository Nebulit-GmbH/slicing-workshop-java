...
3. if the slice is in status planned and not assigned, assign it to backend_worker (property "assigned" ) and continue with prompt_backend.md. Ignore the rest of this file.
4. if the status is in status "InProgress" and assigned to backend_worker, updated started_time and continue with prompt_backend.md. Ignore the rest of this file.
5. if the status is "done" and assigned to "backend_worker", assign the task to "ui_worker" if the frontend folder exists in the root and move it back to status "planned". continue with prompt_frontend.md. Ignore the rest of this file. if prompt_frontend.md does not exist, set status to "done" and stop. Ignore the rest of this loop.
